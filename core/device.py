...
class StreamReaderNode(JOVBaseNode):
    ...
    def run(self, **kw) -> Tuple[torch.Tensor, torch.Tensor]:
        wait = parse_list_value(kw, Lexicon.WAIT, EnumConvertType.BOOLEAN, False)[0]
        if wait:
            return self.__last
        images = []
        batch_size, rate = parse_list_value(kw, Lexicon.BATCH, EnumConvertType.VEC2INT, (1, 30), 1)[0]
        pbar = ProgressBar(batch_size)
        rate = 1. / rate
        width, height = parse_list_value(kw, Lexicon.WH, EnumConvertType.VEC2INT, (MIN_IMAGE_SIZE, MIN_IMAGE_SIZE))[0]
        matte = parse_list_value(kw, Lexicon.MATTE, EnumConvertType.VEC4INT, (0,0,0,255), 0, 255)[0]
        mode = parse_list_value(kw, Lexicon.MODE, EnumConvertType.STRING, EnumScaleMode.NONE.name)[0]
        mode = EnumScaleMode[mode]
        sample = parse_list_value(kw, Lexicon.SAMPLE, EnumConvertType.STRING, EnumInterpolation.LANCZOS4.name)[0]
        sample = EnumInterpolation[sample]
        source = parse_list_value(kw, Lexicon.SOURCE, EnumConvertType.STRING, EnumStreamType.URL.name)[0]
        source = EnumStreamType[source]
        if source == EnumStreamType.MONITOR:
            self.__deviceType = EnumStreamType.MONITOR
            which = parse_list_value(kw, Lexicon.MONITOR, EnumConvertType.STRING, "0")[0]
            ...
            if (which := parse_list_value(kw, Lexicon.WINDOW, EnumConvertType.STRING, "NONE")[0]) != "NONE":
                which = int(which.split('-')[-1].strip())
                dpi = parse_list_value(kw, Lexicon.DPI, EnumConvertType.BOOLEAN, True)[0]
                for idx in range(batch_size):
            ...
            url = parse_list_value(kw, Lexicon.URL, EnumConvertType.STRING, "")[0]
            if source == EnumStreamType.CAMERA:
                url = parse_list_value(kw, Lexicon.CAMERA, EnumConvertType.STRING, "")[0]
            ...
            if self.__device is not None:
                ...
                fps = parse_list_value(kw, Lexicon.FPS, EnumConvertType.INT, 30)[0]
                # if self.__device.fps != fps:
                self.__device.fps = fps

                if type(self.__device) == MediaStreamDevice:
                    self.__device.zoom = parse_list_value(kw, Lexicon.ZOOM, EnumConvertType.INT, 0)[0]

                orient = parse_list_value(kw, Lexicon.ORIENT, EnumConvertType.STRING, EnumCanvasOrientation.NORMAL.name)[0]
...
class StreamWriterNode(JOVBaseNode):
    ...
    def run(self, **kw) -> Tuple[torch.Tensor]:
        route = parse_list_value(kw, Lexicon.ROUTE, EnumConvertType.STRING, "/stream")
        images = parse_list_value(kw, Lexicon.PIXEL, EnumConvertType.IMAGE, None)
        wihi = parse_list_value(kw, Lexicon.WH, EnumConvertType.VEC2INT, (MIN_IMAGE_SIZE, MIN_IMAGE_SIZE), MIN_IMAGE_SIZE)
        matte = parse_list_value(kw, Lexicon.MATTE, EnumConvertType.VEC4INT, (0,0,0,0), 0, 255)
        mode = parse_list_value(kw, Lexicon.MODE, EnumConvertType.STRING, EnumScaleMode.NONE.name)
        sample = parse_list_value(kw, Lexicon.SAMPLE, EnumConvertType.STRING, EnumInterpolation.LANCZOS4.name)
...
if JOV_SPOUT:
    class SpoutWriterNode(JOVBaseNode):
        ...
        def run(self, **kw) -> Tuple[torch.Tensor]:
            images = parse_list_value(kw, Lexicon.PIXEL, EnumConvertType.IMAGE, None)
            host = parse_list_value(kw, Lexicon.ROUTE, EnumConvertType.STRING, "")
            # fps = parse_list_value(kw, Lexicon.FPS, EnumConvertType.INT, 30)
            mode = parse_list_value(kw, Lexicon.MODE, EnumConvertType.STRING, EnumScaleMode.NONE.name)
            wihi = parse_list_value(kw, Lexicon.WH, EnumConvertType.VEC2INT, (MIN_IMAGE_SIZE, MIN_IMAGE_SIZE), MIN_IMAGE_SIZE)
            sample = parse_list_value(kw, Lexicon.SAMPLE, EnumConvertType.STRING, EnumInterpolation.LANCZOS4.name)
            matte = parse_list_value(kw, Lexicon.MATTE, EnumConvertType.VEC4INT, (0,0,0,0), 0, 255)
...
class MIDIMessageNode(JOVBaseNode):
    ...
    def run(self, **kw) -> Tuple[object, bool, int, int, int, float, float]:
        message = parse_list_value(kw, Lexicon.MIDI, EnumConvertType.ANY, None)
        results = []
...
class MIDIReaderNode(JOVBaseNode):
    ...
    def run(self, **kw) -> Tuple[bool, int, int, int]:
        device = parse_list_value(kw, Lexicon.DEVICE, EnumConvertType.STRING, None)
...
class MIDIFilterEZNode(JOVBaseNode):
    ...
    def run(self, **kw) -> Tuple[bool]:
        message = parse_list_value(kw, Lexicon.MIDI, EnumConvertType.ANY, None)[0]
        if message is None:
            logger.warning('no midi message. connected?')
            return (None, False, )

        # empty values mean pass-thru (no filter)
        val = parse_list_value(kw, Lexicon.MODE, EnumConvertType.STRING, MIDINoteOnFilter.IGNORE.name)[0]
        ...
        if (val := parse_list_value(kw, Lexicon.CHANNEL, EnumConvertType.INT, -1)[0]) != -1 and val != message.channel:
            return (message, False, )
        if (val := parse_list_value(kw, Lexicon.CONTROL, EnumConvertType.INT, -1)[0]) != -1 and val != message.control:
            return (message, False, )
        if (val := parse_list_value(kw, Lexicon.NOTE, EnumConvertType.INT, -1)[0]) != -1 and val != message.note:
            return (message, False, )
        if (val := parse_list_value(kw, Lexicon.VALUE, EnumConvertType.INT, -1)[0]) != -1 and val != message.value:
            return (message, False, )
        if (val := parse_list_value(kw, Lexicon.NORMALIZE, EnumConvertType.INT, -1)[0]) != -1 and isclose(message.normal):
            return (message, False, )
        return (message, True, )
...
class MIDIFilterNode(JOVBaseNode):
    ...
    def run(self, **kw) -> Tuple[bool]:
        ...
        # empty values mean pass-thru (no filter)
        val = parse_list_value(kw, Lexicon.ON, EnumConvertType.STRING, MIDINoteOnFilter.IGNORE.name)[0]
        val = MIDINoteOnFilter[val]
        if val != MIDINoteOnFilter.IGNORE:
        ...
        if self.__filter(message.channel, parse_list_value(kw, Lexicon.CHANNEL, EnumConvertType.BOOLEAN, False)[0]) == False:
            return (message, False, )
        if self.__filter(message.control, parse_list_value(kw, Lexicon.CONTROL, EnumConvertType.BOOLEAN, False)[0]) == False:
            return (message, False, )
        if self.__filter(message.note, parse_list_value(kw, Lexicon.NOTE, EnumConvertType.BOOLEAN, False)[0]) == False:
            return (message, False, )
        if self.__filter(message.value, parse_list_value(kw, Lexicon.VALUE, EnumConvertType.BOOLEAN, False)[0]) == False:
            return (message, False, )
        if self.__filter(message.normal, parse_list_value(kw, Lexicon.NORMALIZE, EnumConvertType.BOOLEAN, False)[0]) == False:
            return (message, False, )
        return (message, True, )
